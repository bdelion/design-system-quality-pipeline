...
function anomaliesContent(snapshot: Snapshot, githubUrl?: string): string {
  ...
  const rows = snapshot.normalizedData.anomalies.map((anomaly) => { const component = snapshot.normalizedData.components.find((item) => item.componentId === anomaly.componentId); const linkedPullRequests = anomaly.pullRequestRefs.map((reference) => githubReference(snapshot, reference, githubUrl)).join(' ') || '—'; return `<tr><td><strong>${escapeHtml(anomaly.anomalyId)}</strong><small>${githubReference(snapshot, anomaly.provenance.sourceId ?? 'source inconnue', githubUrl)}</small></td><td>${repositoryReference(snapshot, libraryById.get(component?.libraryId ?? '') ?? 'repository inconnu', githubUrl)}</td><td>${escapeHtml(component?.name ?? anomaly.componentId)}</td><td><span class="tag tag-${anomaly.criticality ?? 'unknown'}">${criticalityLabel(anomaly.criticality)}</span></td><td><span class="state state-${anomaly.status}">${anomalyStatusLabel(anomaly.status)}</span></td><td>${linkedPullRequests}</td><td>${anomaly.everCorrected ? 'Oui' : 'Non'}</td></tr>`; }).join('');
...
function criticalityValue(analytics: Analytics, key: string): number | 'unknown' { return analytics.anomaliesByCriticality[key] ?? 'unknown'; }
